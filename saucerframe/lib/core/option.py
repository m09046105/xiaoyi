...
def script_register(args):
    ...
    # 存放脚本文件路径
    script_path_list = []
    pocs_path = paths.SCRIPT_PATH
    if not (script_name or all_scripts):
        msg = 'Use -s/-As load script/scripts'
        colorprint.red(msg)
        ...
        for root, dirs, files in os.walk(pocs_path):
            ...
            for di in dirs:
                if all_scripts == di:
                    ...
                    conf.module_path=script_path_list
                    #conf.MODULE_NAME = script_name_list
                    #conf.MODULE_FILE_PATH = script_path_list
        if flag == 0:
            colorprint.red('File not %s exist. please check spelling' % all_scripts)
